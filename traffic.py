...
class TrafficModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001):
        ...
        self.result_file = os.path.join("results","traffic","{}_{}.csv".format(model_type,model_size))
        ...
        self.checkpoint_path = os.path.join("tf_sessions","traffic","{}".format(model_type))
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50):
        write_train_loss= open('ltc_train_loss_traffic.txt', 'w')
        ...
        for e in range(epochs):
            ...
            if(verbose and e%log_period == 0):
                write_train_loss.write(str(e)+" "+str(np.round(np.mean(losses),2))+" "+str(np.round(valid_loss,2)) + '\n')
